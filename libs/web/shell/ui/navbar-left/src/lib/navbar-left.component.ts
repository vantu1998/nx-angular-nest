...
@Component({
  ...
  template: `
    <div class="w-[64px] bg-[#0747a6] h-screen flex items-center py-8 flex-col color-white">
      ...
      <div *ngFor="let item of navItems" [matTooltip]="item.tooltip" matTooltipPosition="right" class="my-1">
        <mat-icon class="text-white">{{item.icon}}</mat-icon>
      </div>
      <div class="flex-auto"></div>
      <div matTooltip="Admin user" matTooltipPosition="right" class="border-2 border-solid border-transparent hover:border-blue-500 rounded-full w-10 h-10">
      ...
      <div class="text-white w-8 h-8 my-2 border-4 border-solid border-transparent hover:border-blue-500 rounded-full">
